...
#include <bitset>
#include <regex>
#include <filesystem> // Added for checking file extension
...
int main(int argc, char *argv[]) {
  ...
  // Ensure VM state directory exists (Crucial for GUI)
  setupVmStateDirectory();
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--help" || arg == "-h") {
        std::cout << "Usage: " << argv[0] << " [options]\n"
                  << "Options:\n"
                  << "  <config.json>        Run with specific configuration file (GUI Mode)\n"
                  << "  --help, -h           Show this help message\n"
                  << "  --assemble <file>    Assemble the specified file\n"
                  << "  --run <file>         Run the specified file\n"
                  << "  --verbose-errors     Enable verbose error printing\n"
    ...
    // --- NEW: Handle Config JSON (GUI Integration) ---
    } else if (arg.find(".json") != std::string::npos) {
        try {
            // 1. Load Configuration
            vm_config::config.load(arg);
            std::cout << "Configuration loaded from: " << arg << std::endl;

            // 2. Get Input File from Config
            // Ensure your Config class has this method or accesses the json data directly
            std::string asm_file = vm_config::config.getInputProgramPath(); 
            ...
            RVSSVM vm;
            vm.LoadProgram(program);
            vm.Run(); // This calls Run_Pipelined internally based on mode
            
            return 0; // Exit after GUI run (Do not enter interactive loop)

        } catch (const std::exception& e) {
            std::cerr << "GUI Mode Error: " << e.what() << std::endl;
            return 1;
        }

    } else if (arg == "--start-vm") {
        break; // Proceed to interactive loop below
  ...
  // Only reached if --start-vm is used
